...
public class Paging {
	...
	private int totalCount; //총게시글수(DB에서 조회된 전체 결과의 행수)
	private int listCount;// 한페이지당 출력될 게시글수 (직접 설정할 수 있다)
	private int totalPage;//총 페이지 수 (계산으로 알아낸다)
	
	private int pageCount ;// 한화면에 출력될 페이지 수(직접 설정할 수 있음)
	...
	//총 게시글 수와 현재 페이지, 보여질 게시글 수를입력하는 생성자
	...
	private void makePaging() {
		if(totalCount==0) return; //게시글 없는 경우
		...
		//현재 페이지 보정
		//(총페이지 번호보다 현재 페이지번호가 높을대 총 페이지 번호로 고정)
